...
function buildBannerSection(movie){
   ...
   .then(res => {
    ...
    if(Array.isArray(catagories) && catagories.length){
      catagories.forEach(catagory => {
        fetchandBuildMovieSections(apiPath.fetchMoviesList(catagory.id) 
        , catagory.name);
      });
    }
  ...
  function fetchandBuildMovieSections(fetchUrl, catagoryName){
    ...
    .then(res => {console.log(res.results)
     ...
     if(Array.isArray(movies) && movies.length){
      buildMovieSections(movies, catagoryName)
     }
     return movies;
...
function buildMovieSections(list, catagoryName){
 ...
 //append html inti movie container

 moviesCont.append(div);

}

function searchOnYoutube(movieName , iframeId){
  if(!movieName) return;
